...
namespace Translit.Models.Pages
{
    public class FileConverterModel : IFileConverterModel
    {
        ...
        {
            foreach (var f in files) Files.Enqueue(f);
            Left = Files.Count;
        }
        ...
        private void TranslitWordFile(string filename, bool? ignoreSelectedText)
        {
            ...
            {
                var rNodes = doc.Descendants().Where(n =>
                    n.Name.LocalName == "r" &&
            ...
            using (var db = new LiteDatabase(ConnectionString))
            {
                ...
                for (var i = 0; i < words.Length; i++)
                {
                    ...
                    for (var j = 0; j < 3; j++)
                    {
                        ...
                        // Перебор и замена
                        foreach (var n in nodes) n.Value = n.Value.Replace(cyryllic, latin);
                    }
                ...
                {
                    // Перебор и замена
                    foreach (var n in nodes) n.Value = n.Value.Replace(symbols[i].Cyryllic, symbols[i].Latin);
            ...
            if (isConverted) File.Delete(translitFileName);
        ...
        private void TranslitExcelFile(string filename)
        {
            ...
            var nodes = doc.Descendants().Where(n => n.Name.LocalName == "t").ToList();
        ...
        private void TranslitTxtFile(string filename)
        {
            ...
            if (Settings.Default.TransliterateToAnotherFile)
            {
            ...
            using (var db = new LiteDatabase(ConnectionString))
            {
                ...
                for (var i = 0; i < words.Length; i++)
                {
                    ...
                    for (var j = 0; j < 3; j++)
                    {
                        ...
                        foreach (var item in dc.Content.Find(regex).Reverse()) item.Replace(latin);
                    }
                ...
                for (var i = 0; i < symbols.Length; i++)
                {
                    ...
                    foreach (var item in dc.Content.Find(regex).Reverse()) item.Replace(symbols[i].Latin);
        ...
        private void CreateNewFileFromDirectory(string folder, string filename, bool isConverted)
        {
            ...
            if (Settings.Default.TransliterateToAnotherFile || isConverted) return;
